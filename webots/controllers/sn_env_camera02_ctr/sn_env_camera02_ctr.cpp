...
int main(int argc, char **argv)
{
  ...
  sprintf(dev_name, "sn_env_camera02");
  sprintf(topic_name, "%s/image",dev_name);  
  
  //WbDeviceTag ptcam = wb_robot_get_device("sn_env_camera02");
  ...
  pan_servo = wb_robot_get_device("sn_env_camera02_pan_servo");
  tilt_servo = wb_robot_get_device("sn_env_camera02_tilt_servo"); 
  // init speeds
  ...
  pan_angle = PI/4.;  tilt_angle = PI/4.;
  ...
  //--------------------------------------------------------------------------
  //      Added by jschoi for ROS w/ image_transport, 2012-07-17
  ros::init(argc, argv, "sn_env_camera02_inv");
  
  //int time_step  = wb_robot_get_basic_time_step();
/*  
  WbDeviceTag kinect     = wb_robot_get_device("sn_env_kinect02");
...
*/  
  ...
  //ros::Publisher pub = nh.advertise<sensor_msgs::Image>("sn_env_camera02/image",1);
  ...
  //image_transport::Publisher pub = it.advertise("kinect02/image", 1);
/*
  image_transport::ImageTransport it(nh);
  image_transport::Publisher pub = it.advertise("kinect02/image", 1);
*/ 
...
//////  
    ...
    double pan_angle = wb_servo_get_position(pan_servo);

    if(pan_angle >= 0.99*PI/4.){
      pan_angle = -PI/4.;  tilt_angle = PI/9.;
      wb_servo_set_position(pan_servo, pan_angle);
      wb_servo_set_position(tilt_servo, tilt_angle);
    } else if(pan_angle <= -0.99*PI/4.){
      pan_angle = PI/4.;  tilt_angle = PI/9.;
      wb_servo_set_position(pan_servo, pan_angle);
      wb_servo_set_position(tilt_servo, tilt_angle);
    }    
